...
useSeoMeta({
  title: `Programs Portal | ${siteName}`,
  description: 'Manage core youth, digital, and family support programs.',
})
...
<template>
  <div class="space-y-10">
    ...
    <div class="surface-card rounded-[2.5rem] p-7 sm:p-9 border border-line/80 bg-gradient-to-r from-surface to-surfaceStrong flex flex-col sm:flex-row sm:items-center justify-between gap-6">
      <div>
        <span class="section-kicker text-lime">Dedicated Route: /admin/programs</span>
        <h1 class="font-display text-2xl sm:text-3xl font-bold text-ink mt-1">Programs & Initiatives Management Portal</h1>
        <p class="text-xs text-muted mt-1 max-w-xl">
          Create and update core community pillars. Programs published here immediately appear on the homepage initiative cards and the `/programs` directory.
        </p>
      </div>
      <button
        type="button"
        @click="resetForm()"
        class="rounded-full bg-lime/20 border border-lime/40 px-5 py-2.5 text-xs font-bold text-lime hover:bg-lime hover:text-slate-950 transition shrink-0 self-start sm:self-center"
      >
        + Add New Program Pillar
      </button>
    </div>
      ...
      <div class="lg:col-span-7 space-y-6">
        <div class="surface-card rounded-[2rem] p-6 sm:p-8 border border-line/80">
          <div class="flex items-center justify-between border-b border-line/70 pb-5 mb-6">
            <div>
              <h2 class="font-display text-xl font-bold text-ink">
                {{ editingId ? 'Edit Program Pillar' : 'Create New Program Pillar' }}
              </h2>
              <p class="text-xs text-muted mt-1">
                Enter program title, tagline, description, and cover photo below.
              </p>
            </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            <div class="grid gap-6 sm:grid-cols-2">
              <div class="sm:col-span-2">
                ...
                <input
                  ...
                  type="text"
                  required
                  placeholder="e.g. Youth Digital Mentorship Hub"
                  class="w-full rounded-2xl border border-line/80 bg-surfaceStrong/70 px-4 py-3 text-sm text-ink focus:border-lime focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  placeholder="e.g. Empowering next-gen creators"
                  class="w-full rounded-2xl border border-line/80 bg-surfaceStrong/70 px-4 py-3 text-sm text-ink focus:border-lime focus:outline-none"
                />
              </div>
            ...
            <div>
              <label class="block text-xs font-bold uppercase tracking-wider text-muted mb-2">
                Program Description / Core Goals *
              </label>
              <textarea
                v-model="formContent"
                rows="5"
                required
                placeholder="Provide a detailed overview of what this program initiative achieves for participants..."
                class="w-full rounded-2xl border border-line/80 bg-surfaceStrong/70 px-4 py-3 text-sm text-ink focus:border-lime focus:outline-none"
              />
            </div>
            ...
            <div class="rounded-3xl border border-line/80 bg-surfaceStrong/50 p-6 space-y-4">
              <div class="flex items-center justify-between">
                <label class="block text-xs font-bold uppercase tracking-wider text-ink">
                  Program Cover Photo Upload
                </label>
                <span class="rounded-full bg-lime/20 text-lime px-3 py-1 text-[11px] font-bold">
                  Photo Mode (Max 10MB)
                </span>
              </div>

              <div class="relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-line/80 bg-surface/50 p-6 text-center hover:border-lime transition">
                <input
                  type="file"
                  accept="image/*,video/*"
                ...
                <div class="space-y-2 pointer-events-none">
                  <span class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-surfaceStrong text-lime">
              ...
              <div v-if="uploadFileObj" class="flex items-center justify-between rounded-xl bg-surface p-3 border border-line">
                <div class="flex items-center gap-3">
                  <span class="text-xs font-bold text-lime">Selected file:</span>
              ...
              <div v-if="uploadProgressMsg" class="rounded-xl border border-lime/40 bg-lime/10 p-3 text-xs text-lime font-bold">
                {{ uploadProgressMsg }}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  placeholder="https://..."
                  class="w-full rounded-xl border border-line/70 bg-surface px-3 py-2 text-xs text-ink focus:border-lime focus:outline-none"
                />
              </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-line/70">
              ...
              <button
                ...
                class="rounded-full bg-lime px-8 py-3 text-sm font-bold text-slate-950 transition hover:opacity-90 disabled:opacity-50 shadow-md"
              >
                ...
                <span v-else>{{ editingId ? 'Update Program' : 'Publish Program' }}</span>
              </button>
            </div>
          </form>
        </div>
        ...
        <div class="sticky top-24 space-y-6">
          <div class="surface-card rounded-[2rem] p-6 border border-line/80">
            <div class="flex items-center justify-between mb-4">
              <span class="text-xs font-bold uppercase tracking-wider text-lime">Live Program Card Preview</span>
            ...
            <article class="surface-card interactive-card overflow-hidden rounded-[2rem] border border-line shadow-lg">
              <div class="flex flex-col">
                <div class="relative h-60 w-full overflow-hidden bg-surfaceStrong">
                  <img
                    :src="formMediaUrl || 'https://activegrowthgroups.my.canva.site/_assets/media/113747c97c567daf71b6305fb478fe7a.jpg'"
                    :alt="formTitle"
                    class="h-full w-full object-cover"
                  />
                  <span class="absolute top-3 left-3 rounded-full bg-black/70 px-3 py-1 text-xs font-bold text-lime backdrop-blur-md">
                    {{ formCategory || 'Program Pillar' }}
                  </span>
                </div>

                <div class="p-6 space-y-4">
                  <h3 class="font-display text-xl font-bold text-ink">{{ formTitle || 'Youth Digital Mentorship Hub' }}</h3>
                  <p v-if="formSubtitle" class="text-sm font-semibold text-lime">{{ formSubtitle }}</p>
                  <p class="text-sm text-muted leading-relaxed line-clamp-3">
                    {{ formContent || 'Providing hands-on digital skills training, software access, and career mentorship for youth...' }}
                  </p>
                </div>
              </div>
            </article>
    ...
    <div class="surface-card rounded-[2rem] p-6 sm:p-8 border border-line/80">
      <div class="flex items-center justify-between border-b border-line/70 pb-6 mb-6">
        <div>
          <h3 class="font-display text-2xl font-bold text-ink">
            Published Program Pillars ({{ programs.length }})
          </h3>
          <p class="text-xs text-muted mt-1">
            Click Edit to modify or Delete to instantly remove from the live website.
          </p>
        </div>
      </div>

      <div v-if="programs.length === 0" class="py-12 text-center text-muted">
        No program pillars published yet. Use the form above to add your first initiative.
      </div>
      ...
      <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              <span class="rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-lime/20 text-lime">
          ...
          <div class="flex items-center justify-between pt-3 border-t border-line/60">
            <button type="button" @click="editItem(item)" class="text-xs font-bold text-lime hover:underline">
              Edit Program
            </button>
